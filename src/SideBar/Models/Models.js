...
export const Models = () => {
  ...
  const { newContract, setNewContract, inputTable, result, setResult } =
    useConstants();
          ...
          <div dir="rtl" className="container-AddContract">
            ...
            <div className="search-addcontract" dir="rtl">
              ...
              <div className="addcontr">
                <button className=" button1" onClick={() => setOpenModel(true)}>
                  اضافة طبيعة عقد
                </button>
            ...
            <div className="contractContainer ">
              <div className="flex justify-center text-center w-[100%]">
                <div className="titles" dir="ltr">
                  <div className="t1">
                    <div className="t2 t2-1">رقم العقد</div>
                    <div className="triangle">
                      <img src={triangle}></img>
                    </div>
                  </div>
                  <div className="t1 ml-[9%]">
                    <div className="t2 t2-2">طبيعة العقد</div>
                    <div className="triangle">
                      <img src={triangle}></img>
                    </div>
                  </div>
                  <div className="t1 ml-[8%] ">
                    <div className="t2 t2-3">عدد النماذج</div>
                    <div className="triangle">
                      <img src={triangle}></img>
                    </div>
                  </div>
              ...
              <div className="scrollbar">
                {inputTable
                  .filter((contract) =>
                    contract.natureOfContract.includes(result)
                  )
                  .map((data, index) => {
                    return (
                      <Link
                        className="line-contract hover:bg-[#FFF5DE]"
                        onClick={() => setModel(data.natureOfContract)}
                        key={data.natureOfContract}
                        to={`/نماذج العقود/${data.natureOfContract}`}
                        dir="ltr"
                      >
                        <div className="numberOfContract">{data.number}</div>
                        <div
                          key={data.natureOfContract}
                          className="natureOfContract"
                        >
                          {data.natureOfContract}
                        </div>
                        <div className="numberOfModels">
                          <div className="models">{data.numberOfModels}</div>
                        </div>
                      </Link>
                    );
                  })}
